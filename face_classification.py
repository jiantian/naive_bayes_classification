...
class naiveBayes:
	...
	def getTrainSet(self, training_file, training_label):
		...
		for i in range(len(raw_lables)):
			image_temp = raw_images[i*70:i*70+70]
			for j in range(70):
	...
	def highestPosterori(self):
		for label in range(2):
			...
			print "Test sample from index of "+str(index*70)
	...
	def odd_ratio(self, class1, class2):
		class1_image = np.zeros((70,60))
		class2_image = np.zeros((70,60))
		odd_ratio_image = np.zeros((70,60))
		for x in range(70):
			for y in range(60):
				feature = (x, y, 1)
...
if __name__ == "__main__":
	if len(sys.argv) != 5:
		print "Usage: python part1_extra_face.py training_file training_label test_file test_label"
		sys.exit()
